...
static void rcheevos_award_achievement(const rc_client_achievement_t* cheevo)
{
   ...
   if (settings->bools.cheevos_visibility_unlock)
   {
      ...
      if (rarity < 10.0 && rarity > 0.0)
         msg = MSG_RARE_ACHIEVEMENT_UNLOCKED;
      ...
      if (gfx_widgets_ready())
      {
         ...
         if (rarity > 0.0)
            snprintf(title, sizeof(title), "%s - %0.2f%%", msg_hash_to_str(msg), rarity);
         else
            strlcpy(title, msg_hash_to_str(msg), sizeof(title));
      ...
      {
         char buffer[256];
         size_t _len = strlcpy(buffer, msg_hash_to_str(msg),
               sizeof(buffer));
